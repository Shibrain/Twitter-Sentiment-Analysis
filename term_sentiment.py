    ...
    for t in tweet:
        ...
        for w in t.split(' '):            
            if w not in dic.keys():
                terms[w] = 0
            elif w.encode('utf-8', "ignore") in dic.keys():
                val = val +float(dic[w])

        for term in terms:
            terms[term] = float(val)
            print "%s %.3f"%(term.encode('utf-8'),terms[term])
...
def main():
    ...
    Calculate_terms(tweetParse(tweet_file),dic)
